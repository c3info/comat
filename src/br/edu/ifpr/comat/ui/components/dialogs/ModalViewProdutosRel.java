...
import br.edu.ifpr.comat.ui.components.tables.TbModelRelacionaProduto;

public class ModalViewProdutosRel extends JDialog implements ActionListener {
	...
	private TbModelRelacionaProduto model;

	private JTextField txtSearch;

	private JButton btRecuperar, btCancelar;

	public ModalViewProdutosRel() {
		buildComponents();
	...
	private void buildComponents() {
		...
		grtopPanel.setHorizontalGroup(grtopPanel.createParallelGroup(
			Alignment.TRAILING).addGroup(grtopPanel.createSequentialGroup()
						.addContainerGap()
						...
						.addComponent(cbxCategoria, GroupLayout.PREFERRED_SIZE,	160, GroupLayout.PREFERRED_SIZE).addGap(6)));
		grtopPanel.setVerticalGroup(
				grtopPanel.createParallelGroup(Alignment.LEADING).addGroup(grtopPanel.createSequentialGroup()
						.addContainerGap()
						.addGroup(
							grtopPanel.createParallelGroup(
								Alignment.BASELINE)
								.addComponent(lblBuscar)
		...
		model = new TbModelRelacionaProduto(
				new ProdutoController().searchStatus("Disponível"));
		setModelTable(model);
	...
	private void actionUpdateModel() {
		...
		if (cat.equals(def)) {
			...
			model = new TbModelRelacionaProduto(
					new ProdutoController().searchStatusCategoria("Disponível",
							cat));
			setModelTable(model);
		}
	...
	public TbModelRelacionaProduto getModel() {
		return model;
	}
}
