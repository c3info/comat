...
public class ProdutoDAO extends BaseDAO {
	...
	public int checkByCategoria(int id){		
		int res = 0;
		try {
			trns = session.beginTransaction();
			String queryString = "select count(*) from Produto where idCategoriaFk = :id";
			Query query = session.createQuery(queryString);
	...
	public int checkUsed(int id) {
		int res = 0;
		try {
			trns = session.beginTransaction();
			String queryString = "select count(*) from Produto where refProduto = :id";
			Query query = session.createQuery(queryString);
			query.setInteger("id", id);
			res = ((Number) query.uniqueResult()).intValue();
		} catch (HibernateException hi) {
